...
### Added
- **Claude Code can bill against a Claude Pro/Max subscription** (parallel to Cursor's subscription model) instead of metered API tokens. Run `claude setup-token` on a subscription-logged-in machine, store the resulting `sk-ant-oat…` OAuth token as a secret, and pass it as the action's `api-key`. The action detects the `sk-ant-oat…` prefix and forwards it as `CLAUDE_CODE_OAUTH_TOKEN` (subscription auth); a normal `sk-ant-api…` key is still forwarded as `ANTHROPIC_API_KEY` (metered). No new input — `api-key` accepts either. Codex has no clean CI equivalent (its ChatGPT-subscription auth is an interactive OAuth flow with rotating tokens and likely violates OpenAI's automation terms), so it stays on API-key auth. See `docs/PROVIDERS.md` § "Billing Claude Code against a subscription". **Security:** a subscription token grants broader account access than a scoped key — use it only with `persist-credentials: false` on trusted (non-fork) PRs.
...
### Fixed
- **Agent-runner providers no longer fail the whole review when a CLI writes malformed `findings.json` but leaves a recoverable summary.** PR #11 exposed this with `codex-cli 0.144.4`: Codex completed the review and wrote a useful Markdown summary, but one inline-finding string was not valid JSON, so the parser raised `Malformed findings.json` and the job failed. The subprocess boundary now enables an explicit summary-only fallback: malformed finding objects are dropped, the recovered summary is posted with a note, and the run logs a warning. Direct parser calls remain strict by default.
- **`provider: codex` no longer copies JSON MCP config to an ignored home path.** After switching Codex auth to an isolated per-run `CODEX_HOME`, the old `mcp-config-file` copy still targeted `~/.codex/mcp.json`, which the subprocess ignored and Codex does not read anyway (`config.toml` is the supported path). The Codex provider now warns without copying the ignored JSON file; use `agent-extra-args` / `config.toml` for Codex MCP setup.
- **`provider: codex` no longer fails with `401 Missing bearer or basic authentication`.** Codex CLI 0.122+ stopped reading `OPENAI_API_KEY` from the environment and now reads credentials **only** from `$CODEX_HOME/auth.json`. `codex-cli 0.144.3` (the version currently on npm and what `ubuntu-latest` picks up during install) hit this breakage: every `codex exec` invocation reached `api.openai.com/v1/responses` with an empty Authorization header and 401'd. `CodexProvider` now materializes an apikey-mode `auth.json` (`{"OPENAI_API_KEY": "..."}`) in an isolated per-run `CODEX_HOME` (`tempfile.mkdtemp(prefix="aiprr-codex-")`, mode `0700`, file mode `0600`) before each invocation and removes the whole tempdir in a `finally` block after — so a persistent self-hosted runner with a ChatGPT-mode `~/.codex/auth.json` never overrides our key and no runner accumulates stale api-key state. `OPENAI_API_KEY` continues to be forwarded to the subprocess for back-compat with Codex < 0.122. Regression tests in `CodexAuthJsonTests` cover the env forwarding, on-disk file shape, permission modes, and cleanup. See `docs/PROVIDERS.md` § "Codex auth model (0.122+ requires `$CODEX_HOME/auth.json`)".

### Changed
- **Marketplace listing renamed back to "AI PR Reviewer"** (`action.yml` `name:` reverted from `Dailybot AI PR Reviewer`). The v1.2.1 vendor prefix was a defensive over-fix — it addressed a real slug collision (`ai-pull-request-reviewer`, owned by the third-party `appchoose/ai-pr-review`), but that collision was on the *full-form* title only. The *abbreviated* title "AI PR Reviewer" slugifies to `ai-pr-reviewer` — a distinct slug that was free all along and matches this repo's own slug exactly. Marketplace URL is now `github.com/marketplace/actions/ai-pr-reviewer`; workflow `uses:` pins are unaffected (`DailybotHQ/ai-pr-reviewer@v1` unchanged). Vendor attribution continues via the `author: 'DailybotHQ'` field, which GitHub auto-renders as "by DailybotHQ" beneath the tile — so no attribution is lost. The rename aligns the Marketplace tile with the project's positioning: MIT-licensed, BYOK, zero backend — a community tool rather than a proprietary SaaS. See `AGENTS.md § 9` for the naming history and the third-party incumbent that made this look scarier than it was. **No consumer action required.**
- **Label matching is now case-insensitive.** `label-gate` (and its `label-once` / `label-added-only` trigger logic) compares label names on a lowercased, whitespace-trimmed basis, so `label-gate: ready` is satisfied by `ready`, `Ready`, or `READY`. Applies to `resolve_trigger_action`, `gh_pr_has_label`, and `count_label_events` — removes a foot-gun where a capitalized label silently failed to trigger.
- **Self-review dogfooding is now gated on a `ready` label + run-once.** The dogfood no longer runs on every push; it runs only while the PR carries a `ready` label (case-insensitive) and only **once per label application** (`trigger-mode: label-once`) — remove and re-add `ready` to re-review. A cheap workflow-level label check short-circuits before any job spins up; the critical-file scope split still governs which legs run when triggered. Puts the maintainer in explicit control of dogfood token spend.
- **Explicit, quality-tier default models for the agent-runner providers.** The CLI providers no longer default to `auto` (which deferred to the account default and could silently be Opus at ≈$5/$25) — the action always pins an explicit **quality-tier** model for consumers (the mini/Haiku tiers are reserved for smoke/dogfood passes, never a consumer default): **`claude-code` → `claude-sonnet-4-6`** (the quality/price sweet spot — finds subtle bugs at ~1/5th of Opus cost); **`codex` → `gpt-5.6-luna`** (≈$1/$6 per 1M tokens, the current-gen budget model and OpenAI parallel of the Sonnet-class choice) instead of the now-**deprecated** `gpt-5-codex` (codex-tier ≈$1.75/$14). The `anthropic` default stays `claude-sonnet-4-6` and Cursor stays `auto` (flat-rate/unlimited on Pro). Consumers can pin a cheaper smoke model (`claude-haiku-4-5` ≈$1/$5, `gpt-5.4-mini` ≈$0.75/$4.50) via `model:`. See `docs/PROVIDERS.md` § "Choosing a cost-efficient model".
- **Self-review dogfooding pins cheap models per leg.** Complementing the frequency scoping, each `self-review.yml` matrix leg now pins an explicit cheap model — the always-on Anthropic smoke baseline and the Claude Code leg use `claude-haiku-4-5`, Codex uses `gpt-5.4-mini`, Cursor uses `auto` — so the dogfood spend is deterministic (a smoke review backed by human review + the deeper CLI legs that run on critical changes).
- **Self-review dogfooding is now cost-scoped.** The `anthropic` self-review leg remains the always-on baseline for every PR/push, but the `claude-code`, `cursor`, and `codex` legs now invoke the reviewer only when the diff touches provider-sensitive action/runtime surfaces (`action.yml`, `scripts/reviewer.py`, prompts, core workflow files, or provider/runtime tests). The always-on baseline also runs with `max-turns: 12` instead of the consumer default `30`, avoiding the failure mode where a smoke review burns all 30 Anthropic turns before falling back to a summary. This keeps routine docs/README PRs cheap while preserving full provider dogfooding for changes that can realistically break provider parity.
