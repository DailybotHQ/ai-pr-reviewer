...
### Changed
- **Cost-efficient default models for the agent-runner providers.** The agent-runner CLIs are autonomous coding agents that use far more tokens per review than the bounded chat-completions path, so their defaults now favour cheaper models: **`claude-code` defaults to `claude-haiku-4-5`** (≈$1/$5 per 1M tokens) instead of `auto` — `auto` used the account default and could silently be Opus (≈$5/$25); **`codex` defaults to `gpt-5.4-mini`** (≈$0.75/$4.50) instead of the now-**deprecated** `gpt-5-codex` (codex-tier models run ≈$1.75/$14). Consumers can pin a stronger model via `model:` (`claude-sonnet-4-6` / `gpt-5.6-luna`, or `auto` for Claude Code to defer to the account default). The `anthropic` default stays `claude-sonnet-4-6` (quality/price sweet spot) and Cursor stays `auto` (flat-rate on Pro). See `docs/PROVIDERS.md` § "Choosing a cost-efficient model".
